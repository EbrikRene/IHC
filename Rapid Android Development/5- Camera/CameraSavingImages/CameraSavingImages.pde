...
void draw() {
  ...
  if (!cam.isStarted()) // 1
  {
    ...
    info += "Camara Actual: "+ cam.getCameraID()+"\n"; // 3
    info += "Dimensiones de la imagen: "+ cam.width + // 4
      "x"+cam.height+"\n";     // 5
    info += "Dimensiones de la foto: "+ cam.getPhotoWidth() + // 6
      "x"+cam.getPhotoHeight()+"\n"; // 7
    info += "Estado del Flash: "+ cam.isFlashEnabled()+"\n"; // 8
